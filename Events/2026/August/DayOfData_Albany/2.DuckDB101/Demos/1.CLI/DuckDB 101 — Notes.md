...
> **Working folder:** run these commands from wherever you've placed `local-vscode/data/flights/` (see `2.LocalVSCode/` in this session's Demos folder for the notebook and data-download script).
...
WHERE f.Cancelled = 0
GROUP BY ap.name, ap.municipality, ap.iso_region
